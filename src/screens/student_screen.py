...
from src.screens.pipelines.voice_pipelines import get_voice_embedding
...
from src.screens.components.dialog_enroll import enrol_dialog
    ...
    for i, sub_node in enumerate(subject):
        ...
        def unenroll_button(sid=sid, sub=sub):
            if st.button("Unenroll from this course", type='tertiary', width='stretch',
                icon=":material/delete_forever:", key=f"unenroll_{sid}"):
...
def student_screen():
    ...
    if st.session_state.show_registration:
        ...
        with st.container(border=True):
            ...
            new_name=st.text_input("Enter your name",  placeholder='E.g. Alica Schmidt')


            st.subheader('Optional: Voice Enrolment')
            st.info("Enrol for voice only attendance")


            audio_data=None
            try:

                audio_data=st.audio_input('Record a short phrase like I am present , My name is Chris.')
